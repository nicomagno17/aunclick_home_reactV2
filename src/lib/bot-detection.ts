...
export class BotDetector {
  /**
   * Analiza el comportamiento del usuario para detectar bots
   */
  static async analyzeBehavior(behavior: UserBehavior): Promise<BotDetectionResult> {
    ...
    const ipCheck = await this.checkIPBlacklist(behavior.ip);
    if (ipCheck.isBlacklisted) {
      score += 50; // Penalización alta para IPs en blacklist
      reasons.push(`IP ${behavior.ip} está en lista negra`);
    }
   ...
   */
  private static async checkIPBlacklist(ip: string): Promise<{ isBlacklisted: boolean; reason?: string }> {
    try {
      // En producción, consultar una base de datos de IPs maliciosas
      // Por ahora, verificar patrones conocidos
      const suspiciousPatterns = [
        /^192\.168\./,  // IPs locales (posible VPN)
        /^10\./,        // IPs privadas
        /^172\./,       // IPs privadas
      ];
      ...
      for (const pattern of suspiciousPatterns) {
        if (pattern.test(ip)) {
          return { isBlacklisted: true, reason: 'IP privada o local detectada' };
        }
      }

      // Verificar si hay muchos intentos fallidos desde esta IP
      const recentFailures = await executeQuerySingle(
      ...
      if (recentFailures && recentFailures.count > 10) {
        return { isBlacklisted: true, reason: 'Múltiples fallos recientes desde esta IP' };
      }
  ...
  /**
   * Registra un evento de detección de bot
   */
  static async logBotDetection(
    identifier: string,
    ...
    try {
      await executeQuerySingle(
        `INSERT INTO security_events (
          ...
          NULL, 'bot_detection', ?, ?, ?, ?, NOW()
        )`,
        [
          result.riskLevel,
          `Bot detection: ${result.isBot ? 'BOT' : 'HUMAN'} (confidence: ${(result.confidence * 100).toFixed(1)}%)`,
          JSON.stringify({
            score: result.score,
...
export const analyzeUserForBots = (behavior: UserBehavior) =>
  BotDetector.analyzeBehavior(behavior);

// Hook de React para detección de bots en componentes
export const useBotDetection = () => {
  const analyzeBehavior = async (behavior: UserBehavior) => {
    const result = await BotDetector.analyzeBehavior(behavior);

    // Log the detection result
    await BotDetector.logBotDetection(
      behavior.ip,
      result,
      behavior
    );
